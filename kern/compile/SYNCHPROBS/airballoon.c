...
marigold(void *p, unsigned long arg)
{
 ...
 int balloonIndex, groundIndex; 
	while(numDisconnected < NROPES){
		...
		lock_acquire(balloonHooks[groundStakes[groundIndex].balloonIndex].hook_lk);
		...
		lock_release(balloonHooks[groundStakes[groundIndex].balloonIndex].hook_lk);
	...
	while(numDisconnected < NROPES){
        ...
        if(&balloonHooks[groundStakes[groundSwitch2].balloonIndex].hook_lk > 
		   &balloonHooks[groundStakes[groundSwitch1].balloonIndex].hook_lk){
				lock_acquire(balloonHooks[groundStakes[groundSwitch2].balloonIndex].hook_lk);
				lock_acquire(balloonHooks[groundStakes[groundSwitch1].balloonIndex].hook_lk);
			}else{
				lock_acquire(balloonHooks[groundStakes[groundSwitch1].balloonIndex].hook_lk);
				lock_acquire(balloonHooks[groundStakes[groundSwitch2].balloonIndex].hook_lk);
			}
		//this is the case where there are no more threads to switch
		// if(NROPES - numDisconnected < 2){
		//   lock_release(balloonHooks[groundStakes[groundSwitch1].balloonIndex].hook_lk);
       	//   lock_release(balloonHooks[groundStakes[groundSwitch2].balloonIndex].hook_lk);
		// 	break;
		// }
...
panic:
	 ...
	 */
	lock_acquire(thread_lk);
	kprintf("dand is %s\n", dand_is_finished? "true" : "false");
	kprintf("mari is %s\n", mari_is_finished? "true" : "false");
	...
	kprintf("dand3 is %s\n", dand_is_finished? "true" : "false");
	kprintf("mari3 is %s\n", mari_is_finished? "true" : "false");
	kprintf("ball3 is %s\n", balloon_is_finished? "true" : "false");
	kprintf("index is %d\n",balloonHooks[1].groundIndex);
